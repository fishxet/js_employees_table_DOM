...
const MIN_AGE = 18;
const MAX_AGE = 90;
const formatValue = (element)=>new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "USD",
...
inputList.forEach((item)=>{
    ...
    } else if (textValue === "age" || textValue === "salary") input.type = "number";
...
[
    ...tableHeadElement.cells
].forEach((x, index)=>{
    x.addEventListener("click", ()=>{
        if (lastClickedElement === x) sorted = !sorted;
        else {
            sorted = false;
        ...
        rows.sort((element1, element2)=>{
            ...
            firstCell = firstCell.includes("$") ? Number(firstCell.replace("$", "").replaceAll(",", "")) : firstCell;
            secondCell = secondCell.includes("$") ? Number(secondCell.replace("$", "").replaceAll(",", "")) : secondCell;
            return sorted ? sortByDESC(firstCell, secondCell) : sortByASC(firstCell, secondCell);
        });
...
tableBodyElement.addEventListener("dblclick", (e)=>{
    ...
    if (activeInput) {
        ...
        prevCell.textContent = activeInput.value === "" ? activeInput.defaultValue : activeInput.value;
    }
    ...
    inputElement.focus();
    const save = ()=>{
        cell.textContent = inputElement.value === "" ? initialValue : inputElement.value;
    };
    inputElement.addEventListener("keydown", (inputEvent)=>{
        if (inputEvent.key === "Enter") save();
        if (inputEvent.key === "Escape") cell.textContent = initialValue;
    });
...
formElement.addEventListener("submit", (e)=>{
    ...
    if (!formName || formName.length < 4) {
        pushNotification(10, 10, "Error: Name is too short", 'Field "Name" must be at least 4 characters long', "error");
        return;
    }
    if (!formPosition) {
        pushNotification(10, 10, "Error: Position is empty", 'Field "Position" cannot be empty', "error");
        return;
    }
    if (!formOffice) {
        pushNotification(10, 10, "Error: Office is required", "Please select an office location", "error");
        return;
    }
    if (Number.isNaN(formAge) || formAge < MIN_AGE) {
        pushNotification(10, 10, "Error: Age is too low", `Age must be at least ${MIN_AGE}`, "error");
        return;
    }
    if (formAge > MAX_AGE) {
        pushNotification(10, 10, "Error: Age is too high", `Age must be at most ${MAX_AGE}`, "error");
        return;
    }
    if (!salary || isNaN(Number(salary))) {
        pushNotification(10, 10, "Error: Invalid Salary", "Salary must be a valid number", "error");
        return;
    }
    const tableRow = document.createElement("tr");
    inputList.forEach((key)=>{
        const tableData = document.createElement("td");
        if (key === "salary") tableData.textContent = formatValue(formData.get(key));
        else tableData.textContent = formData.get(key);
        tableRow.appendChild(tableData);
    });
    tableBodyElement.appendChild(tableRow);
    pushNotification(10, 10, "Employee added successfully.", "All information was correct, data was added to the table.", "success");
    formElement.reset();
});
